              ...
              <Form ref="formCustom"
                ...
                <div class="main">
                  ...
                  <FormItem label="税务登记号">
                    <Input type="password"
                           v-model="formAllData.taxRegistrationCertNumber"></Input>
                  </FormItem>
                </div>
                <FormItem>
                  <Button type="primary"
...
export default {
  data () {
    return {
      ...
      //实际控制企业营业期限日期开关beneficiaryIdCard
  ...
  methods: {
      ...
      this.$http.post(this.url, fileFormData, requestConfig).then((res) => {
        ...
        if (res.data["code"] == "200") {
          ...
          if (sign == 'businessLicenseCopy') {
            //营业执照信息图片上传
            console.log(this.formAllData["businessLicenseCopy"]);
            //获取存储成功的图片id
    ...
    //保存表单
    handleSubmit () {
      this.formAllData['businessLicenseValidTime'] = this.newdate(this.businessLicenseStartTime) + ',' + (this.businessLicenseEndTime == null ? "长期" : (this.newdate(this.businessLicenseEndTime)));
      this.formAllData['organizationCertValidTime'] = this.newdate(this.organizationCertStartTime) + ',' + (this.organizationCertEndTime == null ? "长期" : (this.newdate(this.organizationCertEndTime)));
      ...
      this.formAllData['holdingCompanyLicenseValidTime'] = this.newdate(this.holdingCompanyLicenseStartTime) + ',' + (this.holdingCompanyLicenseEndTime == null ? "长期" : (this.newdate(this.holdingCompanyLicenseEndTime)));
