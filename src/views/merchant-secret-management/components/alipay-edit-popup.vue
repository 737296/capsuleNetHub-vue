<template>
  ...
  <Pupup v-model="popup.show" :loading="popup.loading" class-name="popup-model" width="none" >
    <template slot="header"> 支付宝-商户号 <span style="color: red;">(仅维护支付宝1.0)</span> </template>
    <template slot="body">
...
const getDefEditData = () => [
  ...
  [
    ...
    { span: 1 },
    {
      ...
      placeholder: '请输入支付公钥',
      value: '',
      // rule: [{ required: true, message: '请输入支付公钥', trigger: 'blur' }],
      options: []
    }
  ],
  [
    ...
    { span: 1 },
    {
      ...
      value: '',
      rule: [{ required: true, message: '请输入支付宝账号', trigger: 'blur' }],
      options: []
    }
  ]
]
...
export default {
  components: {
    MerchantInfo: (resolve) => require(['./alipay-merchant-info.vue'], resolve)
  },
  data () {
    return {
      popup: getDefPopup(),
